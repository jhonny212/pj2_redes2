...
IFB = "sudo modprobe ifb numifbs=2"
...
COMANDO_IN_ENLACE = f"/usr/sbin/tc  class add dev {INTERFACE_IN} parent 1: classid 1:10 htb rate 1000kbit"
COMANDO_EXTRA_IN = f"tc qdisc add dev {INTERFACE_IN} parent 1:10 handle 10: sfq perturb 10"

#Creando enlace para subida
COMANDO_OUT_ROOT = f"/usr/sbin/tc  qdisc add dev {INTERFACE_OUT} root handle 1: htb"
COMANDO_OUT_ENLACE = f"/usr/sbin/tc  class add dev {INTERFACE_OUT} parent 1: classid 1:10 htb rate 512kbit"
COMANDO_EXTRA_OUT = f"tc qdisc add dev {INTERFACE_OUT} parent 1:10 handle 10: sfq perturb 10"
